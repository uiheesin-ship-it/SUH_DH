...
function columns() {
  ...
  // 잔차 먼저 — 테마 동행이 이 페이지의 1순위 용도라서.
  for (const w of WINDOWS) cols.push({ key: `res${w}`, label: `잔차 ${w}일`, cls: "c res", num: true });
  for (const w of WINDOWS) cols.push({ key: `raw${w}`, label: `원시 ${w}일`, cls: "c raw", num: true });
...
function expand(ticker) {
  ...
  const base = (j) => {
    const m = meta(j);
    return { ticker: DATA.tickers[j], name: m[0], sector: m[1] || "—", industry: m[2] || "—",
...
function render() {
  ...
  $("#tbody").innerHTML = rows.map((r) => `<tr>${cols.map((c) => {
    const v = r[c.key];
    if (c.key === "ticker")
      return `<td class="t"><a href="#" data-go="${esc(v)}">${esc(v)}</a></td>`;
    ...
    return `<td class="c" style="${corrStyle(v)}">${fmtCorr(v)}</td>`;
  }).join("")}</tr>`).join("");
...
$(".presets").addEventListener("click", (e) => {
  ...
  if (b.dataset.preset === "theme") { sortKey = "res50"; sortDir = -1; }
