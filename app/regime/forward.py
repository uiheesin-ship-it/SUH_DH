...
mean and the median. Two details matter:

* the **baseline** uses a moving-block bootstrap with block length = horizon,
  because overlapping forward windows are heavily autocorrelated and an i.i.d.
  bootstrap would understate the interval;
* the **matched** sample is de-clustered, so an i.i.d. bootstrap is appropriate
  — unless the de-clustering gap is shorter than the horizon, in which case the
  windows still overlap and the result flags it.
"""
...
def bootstrap_ci(sample: Sequence[float], *, n_boot: int = 2000, ci_level: float = 0.95,
                 seed: int = 20240101, block: int = 1) -> dict[str, tuple[float, float]]:
    """Percentile bootstrap CI for the mean and the median.

    ``block > 1`` switches to a moving-block bootstrap (overlapping forward
    windows), which keeps the local autocorrelation inside each resample.
    """
    x = np.asarray([v for v in sample if v is not None and np.isfinite(v)], dtype=float)
    n = x.size
    if n < 3:
...
def analyze(close: pd.Series, match_dates: Sequence[pd.Timestamp], fwd_params, *,
            baseline_mask: pd.Series | None = None, min_gap: int = 0) -> dict[int, HorizonResult]:
    ...
    dates = pd.DatetimeIndex([d for d in match_dates if d in fwd.index])
    ...
    for h in horizons:
        ...
        m_ret = fwd.loc[dates, col] if len(dates) else pd.Series(dtype=float)
        m_mdd = mdd.loc[dates, mcol] if len(dates) else pd.Series(dtype=float)

        matched = summarize(m_ret, m_mdd, n_boot=fwd_params.bootstrap_samples,
                            ci_level=fwd_params.ci_level, seed=fwd_params.seed, block=1)
        ...
        if matched["n"] < int(fwd_params.min_sample_warn):
            res.warnings.append(
                f"표본이 {matched['n']}개뿐입니다 (경고 기준 {fwd_params.min_sample_warn}개) — "
                "평균·승률을 신뢰하기 어렵습니다.")
        if min_gap and matched["n"] > 1 and int(min_gap) < h:
            res.warnings.append(
                f"de-clustering 간격({min_gap}일)이 horizon({h}일)보다 짧아 forward 구간이 겹칩니다 — "
                "유효 표본 수는 표시된 n 보다 적습니다.")
        if matched["n"] and not res.ci_excludes_zero:
            res.warnings.append("평균의 95% 신뢰구간이 0을 포함합니다 (방향성 결론 불가).")
        out[h] = res
    return out
