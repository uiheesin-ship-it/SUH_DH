...
```
  ...
  단순 합            8,644        51분     ← 캐시 TTL 30분이던 시절의 실질 비용
```

같이 넣은 캐시 TTL 수정(`SUH_DH_BASE_BARS_TTL`, `daily.yml`)이 중복 수신 **24분**을
없앴으므로, 둘을 합치면 **전수로 바꾸고도 예전보다 빠릅니다(51분 → 47분).**
