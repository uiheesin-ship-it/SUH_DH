...
**후보 솎아내기는 끕니다.** 평평 스크리너는 Finviz 결과를 3,000종목(+ETF 700)으로
잘라 쓰는데, 시총 구간이 고르게 남도록 **일정 간격으로 솎아냅니다.** 구간 안의 어떤
종목이 빠질지는 사실상 임의라서, 평평 쪽처럼 "쓸 만한 표본"만 필요하면 괜찮지만
상관 분석에서는 **사용자가 찾는 바로 그 종목이 조용히 없을 수 있습니다** — 실제로
APPS(디지털터빈, 시총 $1.4B, 60일 거래대금 $29M)가 기준을 다 넘고도 이렇게 빠졌습니다.
그래서 수집기는 상한을 0(무제한)으로 덮어쓰고 가져옵니다. 거르는 일은 위의 유동성
기준에만 맡깁니다.
