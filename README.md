...
| 함정 | 어떻게 다루나 |
|---|---|
| 전환사채를 장기차입금 위에 **또** 더한다 | `LongTermDebtNoncurrent` 는 전환사채를 **이미 담은 합계**다. 개념마다 버킷을 두고 한 버킷에서는 잡히는 **첫 태그 하나만** 쓴다 — 전환사채 태그는 장기차입금 태그가 아예 없는 회사에서만 쓰인다 |
| `…AndCapitalLeaseObligations` 위에 금융리스를 **또** 더한다 | 그 태그가 쓰였으면 금융리스 버킷을 건너뛴다 |
...
| 몇 분기 전 재무상태표가 조용히 붙는다 | 분기말에서 **100일** 밖은 끌어오지 않는다 |
