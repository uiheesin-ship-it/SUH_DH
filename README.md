...
2. 저장소 폴더에서:
   ```bash
   cp .env.example .env     # 그리고 DART_API_KEY= 뒤에 키를 붙여 넣기
   ```
...
> **메모장 함정.** `cp` 를 건너뛰고 메모장으로 새로 만들면 `.env.txt` 가 됩니다 — 메모장이
> 기본으로 `.txt` 를 붙이고 탐색기는 확장자를 숨기기 때문에 눈으로는 `.env` 로 보입니다.
> `./run.sh` 가 그 파일을 발견하면 알려 주고 고치는 법(`mv .env.txt .env`)을 찍습니다.
> 확인은 `ls -la .env && cat .env`.
