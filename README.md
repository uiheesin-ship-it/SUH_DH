...
**갱신 주기 — 주 3회(월·수·금 21:40 UTC).** 미국장 마감 1시간 40분 뒤라 **그날 종가**가
들어오고, 한국시간으로 이튿날 아침에 화면에 반영됩니다(실측: 9/23 수 실행 → `asof`
2026-09-23, 커밋 9/24 00:17 UTC).

이 화면은 **요청할 때 자료를 받아오지 않습니다.** 종목 하나짜리인 분기실적/PER 과 달리
미리 만들어 둔 `data/correl.json` 을 브라우저가 내려받아 계산합니다. 그럴 수밖에 없는
이유는 **가격 수신이 2시간 넘게 걸리기 때문**입니다 — 원본 7,948 종목의 1년치 일봉을
받아 유동성으로 3,000여 개를 추린 뒤에야 행렬이 나옵니다. 계산 자체는 1초대이고,
Yahoo 레이트 리밋을 피하느라 150개씩 쉬면서 3라운드 재시도를 도는 시간이 대부분입니다.

처음엔 수·토 주 2회였습니다. "상관은 천천히 변하니 매일 돌릴 이유가 없다" 가 근거였는데
**평균적으로만** 맞는 말이었습니다. 수요일 낮에 열면 지난 금요일 종가 기준이라 월·화·수
3거래일이 빠졌습니다. 상관이 깨지는 순간을 보려는 화면인데 정작 크게 움직인 그 하루가
빠지는 게 문제입니다. 월·수·금이면 **최대 1거래일**만 뒤처집니다(화요일엔 월요일 종가,
목요일엔 수요일 종가, 주말엔 금요일 종가 — 주말은 장이 없으니 뒤처진 게 아닙니다).

매일까지 안 가는 이유는 돈이 아닙니다(public 레포라 Actions 시간은 무료·무제한).
1.3MB 파일이 실행마다 커밋돼 **깃 저장소가 불어나기 때문**입니다. 시크릿은 필요 없습니다.
...
#### 12M Forward EV/EBITDA — 같은 계단 위에서

PER 은 분자가 주가 하나지만 EV 는 분자를 **매 시점 다시 만들어야** 합니다.

```
EV(t) = 주가(t) × 발행주식수 + 총차입금 − 현금성자산 + 비지배지분 + 우선주
12M forward EV/EBITDA = EV(t) ÷ (t 이후 4개 분기 EBITDA 합)
```

재무상태표는 분기에 한 번 바뀌므로 **PER 과 같은 실적발표일**마다 계단을 밟고 그 사이에는
주가만 움직입니다. 야후의 분기 재무상태표는 **7분기**뿐이라 5년을 못 그립니다 — EDGAR 는
**59~70분기**가 잡혀서(실측 `tools/ev_probe.py`) EDGAR 를 씁니다.

재무상태표 쪽에서 **조용히 틀리는 길이 셋** 있어 전부 막고 테스트로 못 박았습니다.

| 함정 | 어떻게 다루나 |
|---|---|
| 전환사채를 장기차입금 위에 **또** 더한다 | 대개 `LongTermDebtNoncurrent` 안에 이미 있다. 그렇다고 늘 빼면 반대로 누락된다 — 실측(SMCI): 장·단기 합계 4.06B 과 전환사채 4.66B 이 **별개**였고 둘을 더해야 야후 값과 맞았다. **합계 ≥ 전환사채면 빼고, 작으면 더한다** (합계가 부분보다 작을 수는 없다) |
| 회사가 자금조달 수단을 바꾼 구간이 통째로 **0** 이 된다 | 회사 전체에서 태그 하나를 고르면 그렇게 된다. 실측(SMCI): 옛 은행 차입 태그가 버킷을 차지해 지금의 전환사채 9B 가 통째로 빠졌고 EV 가 야후보다 **41% 작게** 나왔다. **날짜별로** 우선순위를 다시 매기면 양쪽 구간이 다 맞는다 |
| `…AndCapitalLeaseObligations` 위에 금융리스를 **또** 더한다 | **그 날짜에** 그 태그가 쓰였으면 금융리스 버킷을 건너뛴다 |
| `StockholdersEquityIncluding…NoncontrollingInterest` 를 비지배지분으로 쓴다 | 이건 **자본 총계**다. `MinorityInterest` **하나만** 쓴다 — 아니면 EV 가 자본만큼 부푼다 |
| 없는 태그를 0 으로 채운다 | 채우지 않는다. PLD 는 `LongTermDebt` 하나뿐이고 유동차입금·CP·전환사채·리스부채 태그가 **아예 없다**(실측). 무엇이 없었는지 차트 아래에 적는다 |
| 우선주를 **액면**으로 본다 | 액면(`PreferredStockValue`)은 우선주가 있어도 0 인 회사가 흔하다. 장부금액(`PreferredStockIncludingAdditionalPaidInCapital`)을 먼저 쓴다 — 실측(SMCI): 액면 0 · 장부 4.23B 이고 그게 야후 EV 와의 차이와 **정확히 같았다**. 상환우선주 등 임시자본도 따로 더한다 |
| 몇 분기 전 재무상태표가 조용히 붙는다 | 분기말 **이전 100일** 안만 본다 |
| **다음 분기** 재무상태표가 끌려 들어온다 | 뒤로는 **45일**까지만 연다. 그 이상 열면 그 발표 시점에 몰랐던 값이 과거 차트에 새어 든다. 45일은 표지(dei) 발행주식수가 제출일 기준이라 필요하다 |

현금은 **현금 + 단기투자자산**입니다 — 실측(AAPL 2026-06-27)으로 이 조합이 야후
`totalCash` 와 **정확히 일치**했습니다. 차입금은 야후보다 큽니다(AAPL 기준 12.5B).
**리스부채를 넣기 때문이고 의도한 차이**입니다. 다만 EBITDA 에서 리스비용을 되돌리지는
않으므로 리스가 큰 회사(유통·항공)의 배수는 높게 나옵니다.

> **점선은 컨센이 아니라 가정입니다.** EBITDA 컨센을 주는 무료 출처가 없습니다(야후·
> Alpha Vantage 에는 항목 자체가 없고 FMP 는 유료). 그렇다고 가장 궁금한 최근 1년을 통째로
> 비울 수는 없어서, **매출 컨센을 PER 과 똑같은 계절성 규칙으로 나눈 뒤 최근 8분기 EBITDA
> 마진의 중앙값**을 곱합니다. 차트 아래에 그 마진과 최근 최저~최고 범위를 같이 적습니다.
> 매출 컨센이 없거나 마진을 못 구하면 **점선을 아예 그리지 않습니다.**

은행·보험은 영업이익 개념이 없어 EBITDA 도, EV/EBITDA 도 만들지 않습니다 — 그 이유가
화면에 그대로 뜹니다.

#### 국장(한국)도 같은 페이지에서 — 재료만 다릅니다

왼쪽 위 **🇺🇸 미국 / 🇰🇷 한국** 토글로 고릅니다. 한국은 **여섯 자리 종목코드**로 찾습니다
(삼성전자 `005930`). 창을 만들고 PER 을 내는 규칙(`app/forwardper.py`)은 **한 줄도 안 고치고
그대로** 씁니다 — 실적발표일에 계단을 밟는다는 생각, 계절성 배분, 실선/점선의 뜻이 시장과
무관하기 때문입니다.

| | 미장 | 국장 |
|---|---|---|
| 분기 실적 | EDGAR XBRL companyfacts | DART `fnlttSinglAcntAll` (연결 CFS, 없으면 별도 OFS). 실측 5년 **18/20 보고서** |
| 실적발표일 | 야후 `get_earnings_dates` | **거래소 〈연결재무제표기준영업(잠정)실적〉 공시일** — 정기보고서 접수일보다 **2~5주 빠릅니다** |
| 컨센서스 | 야후 — 분기 **2** + 연간 **2** | 네이버 — 분기 **1** + 연간 **1**. 대신 **영업이익 컨센이 나옵니다** |
| 주가 | 야후 일봉 | 네이버/KRX 일봉(FinanceDataReader) |

국장에서 다르게 다루는 것들 — 전부 실측으로 확인하고 테스트로 못 박았습니다.

| 함정 | 어떻게 다루나 |
|---|---|
| 손익의 `thstrm_amount` 를 **누적으로 착각** | 이건 **당기 3개월**이다. 실측으로 반기 누적 = 1분기 + 당기가 딱 맞는다. 착각하면 2·3분기가 부풀어 오른다 |
| 사업보고서의 연간값을 그대로 Q4 로 씀 | 미장과 똑같이 `Q4 = 연간 − 3분기 누적`. 3분기 누적은 3분기보고서의 `thstrm_add_amount` |
| **현금흐름표는 누적** | 차분해서 분기값을 만든다. 감가상각이 거기 있어 EBITDA 에 영향을 준다 |
| 같은 이름이 여러 재무제표에 있다 | 비지배지분은 BS·CIS·CF 에 다 있다. 계정을 고를 때 **어느 재무제표인지를 반드시 좁힌다** |
| 회사마다 계정 이름이 다르다 | 표준 계정코드(`ifrs-full_…`)를 이름보다 먼저 쓴다. 미장과 달리 회사가 태그를 갈아타지 않아 오히려 깔끔하다 |

**컨센 지평이 좁은 게 국장의 진짜 제약입니다.** 12개월이면 네 분기가 필요한데 네이버는
앞으로 한 분기와 한 회계연도만 줍니다. 그래서 세 단계로 채우고, **뒤로 갈수록 가정이
세집니다.** 무엇으로 몇 분기를 채웠는지 차트 아래에 그대로 적습니다.

```
① 네이버 분기 컨센이 있는 분기는 그대로            (가정 없음)
② 올해 FY 컨센의 잔여를 계절성 비중으로 배분       (미장과 같은 규칙)
③ 그다음 회계연도 = 같은 분기 1년 전 × 성장률      (가정)
   성장률 = 올해 FY 컨센 ÷ 작년 FY 실적 합
```

> ③ 은 **컨센이 아니라 가정**입니다. 미장은 연간 컨센을 둘 주니 여기까지 갈 일이 없었는데,
> 국장은 하나뿐이라 안 그러면 **가장 최근 1년이 통째로 빕니다.** 성장률을 그대로 이어 쓴다는
> 뜻이라 성장률이 꺾이는 해에는 빗나갑니다. ③ 의 재료(1년 전 같은 분기)마저 없으면
> **그 분기는 비웁니다** — 지어내지 않습니다.
>
> 성장률이 **0.5~2.0배 밖이면 성장 없음(1.0배)으로 물러섭니다.** 실측(삼성전자)에서 올해
> 성장률이 **7.25배**였습니다 — 메모리 사이클 정점이라 실제로 그렇지만, 그 배수를 내년에도
> 곱하면 2028년 분기 EPS 가 56만 원이 됩니다. 컨센이 한 번도 말한 적 없는 숫자이고, 한 해의
> 정점을 영구 성장률로 바꿔 쓰는 셈입니다. 이 가정이 틀리면 forward PER 이 **보수적으로
> (높게)** 나옵니다 — 낙관 쪽으로 틀리는 것보다 낫습니다. 손잡이는
> `app/krquarterly.py` 의 `GROWTH_BAND` 한 줄입니다.

#### 미장과 국장이 다른 곳 — 전부, 그리고 왜

두 시장을 같은 코드로 그리지만 다른 곳이 남습니다. **다른 데는 이유가 있어야 합니다.**
전부 적고 셋으로 나눕니다.

**① 재료가 달라서 — 같게 만들 수 없는 것**

| 항목 | 미장 | 국장 | 왜 |
|---|---|---|---|
| EPS 기준 | GAAP · 조정 **두 벌** | **한 벌**(보고값) | 한국은 회사가 따로 조정 EPS 를 내는 관행이 없습니다. 없는 기준을 만들면 그게 지어낸 숫자입니다 |
| 컨센 지평 | 분기 2 + 연간 2 | 분기 1 + 연간 1 | 네이버가 주는 만큼(실측 2026-09-19) |
| 추정 채우기 | 2단계 | **3단계**(+성장률 이월) | 지평이 좁아 안 그러면 최근 1년이 통째로 빕니다 |
| 영업이익 컨센 | **없음** | **있음** | 야후·AV 에 항목이 없고 FMP 는 유료. 네이버는 그냥 줍니다 |
| 순이익 컨센 | EPS × 주식수 **파생** | **직접** | 네이버가 당기순이익 컨센을 그대로 줍니다 |
| 지배주주순이익 행 | 없음 | **있음** | US GAAP `NetIncomeLoss` 는 이미 지배주주 귀속 |
| **손으로 고치는 단위** | **EPS** | **당기순이익** | 그 시장에서 실제로 보는 숫자이고 컨센도 그 단위입니다. 국장은 실적에서 잰 환산계수로 EPS 와 오갑니다 |
| EBITDA 의 감가상각 | XBRL 분기값 + 빈 자리만 누적 차분 | 현금흐름표 **누적 차분만** | DART 는 감가상각을 현금흐름표에만 싣습니다 |
| 계정 고르기 | 회사가 태그를 갈아타 **날짜별 우선순위** 필요 | 표준 계정코드가 안정적 | 미장에서 SMCI 가 은행차입→전환사채로 갈아탄 사고가 있었습니다 |
| 뒤처짐 경고 | 200일 | 250일 | 국장 분기보고서 마감이 45일 |
| 회계연도 말 | 연간 사실의 실제 `end` | 사업보고서 기간의 끝 | 미장은 52/53주 회계연도가 있습니다(애플) |

**② 이유 없이 달랐던 것 — 고쳤습니다**

| 무엇이 달랐나 | 어떻게 고쳤나 |
|---|---|
| **분기 실적을 손으로 고치는 칸이 국장에 없었다** | "조정·GAAP 구분이 없다" 와 달리 재료 문제가 아니었습니다. **당기순이익**으로 고치게 했습니다 — 화면은 단위를 모른 채 돌고 서버가 `unit`/`values`/`to_eps`/`digits` 로 단위를 정합니다 |
| **컨센을 못 받으면 국장만 조용히 넘어갔다** | 미장처럼 이유를 `notes` 에 적습니다. 안 그러면 컨센 칸이 빈 이유를 알 길이 없습니다 |
| **EV/EBITDA 버튼이 이유 없이 회색이었다** | 못 만드는 이유를 응답에 실어 화면이 말하게 했습니다. 미장은 원래 그렇게 합니다 |
| **매출을 "먼저 잡히는 쪽" 으로 골랐다** | 미장에서 은행 매출이 조용히 2014년에서 끊겼던 바로 그 실수입니다. 이제 국장도 후보를 다 만들고 **가장 최근까지 이어지는 쪽**을 통째로 고릅니다(`_fresher` 를 공유) |
| 미장에서 베껴 온 **죽은 코드**가 남아 있었다 | 야후 발표일 집계 — 국장엔 야후가 없습니다. 제거 |

**③ 아직 못 한 것**

- **EV/EBITDA 가 국장에 없습니다.** 재료(차입금·리스부채·현금·비지배지분·우선주자본금)는
  DART 전체 재무제표에 표준 계정코드로 다 있는 것을 확인했지만, **발행주식수**는 다른
  API(`stockTotqySttus`)로 따로 받아야 해서 다음 차례입니다.
- **가중평균주식수 행**이 국장 표에 없습니다. DART 표준 계정에 없고 주석에 있습니다.
  대신 편집칸이 `EPS ÷ 당기순이익` 에서 역산한 **지배주주 기준 주식수**를 적어 줍니다.
- **정정공시의 최초 보고값**을 미장은 들고 있고 국장은 없습니다. DART 는 (연도, 보고서)별로
  **최신 접수본**만 주므로 최초값을 보려면 접수번호를 따라가야 합니다. 다만 **지금은
  미장에서도 화면에 안 쓰므로** 보이는 차이는 없습니다.
- PER 을 그리는 **최소 분기 수**가 다릅니다(미장은 하나라도 있으면, 국장은 8분기 이상).
  국장 쪽이 엄격한데, 짧은 계열로 그린 PER 은 어느 시장에서든 의미가 없어 미장을 국장에
  맞추는 게 맞습니다.
...
| `app/evebitda.py` | 재무상태표 → EV 구성 → EV/EBITDA 계열 | ✕ |
| `app/krdart.py` | DART 정기보고서·공시 접수일 | ○ |
| `app/krconsensus.py` | 네이버 컨센·실적(분기/연간) | ○ |
| `app/krfundamentals.py` | 정기보고서 → 분기 시계열(누적 차분·Q4 역산) | ✕ |
| `app/krquarterly.py` | 국장 조립 | (위를 부름) |
...
#### 국장(🇰🇷)을 쓰려면 — DART 키, 한 번만

미장은 키가 필요 없습니다. **국장만** DART 무료 API 키가 필요합니다.

1. <https://opendart.fss.or.kr> → 회원가입 → **인증키 신청**. 메일로 40자리 키가 옵니다(무료, 1분).
2. 저장소 폴더에서 **이 한 줄**:
   ```bash
   ./set-dart-key.sh        # 키를 붙여 넣고 Enter
   ```
   앞뒤 공백·따옴표·윈도우 줄끝이 묻어와도 떼어 내고, 40자가 아니면 되묻습니다.
   키 값은 화면에 찍지 않습니다. (손으로 적고 싶으면 `cp .env.example .env` 후 편집)
3. `./run.sh` — 맨 위에 두 줄이 찍히면 됩니다(키 값 자체는 절대 안 찍습니다).
   ```
   .env 에서 환경변수 1개를 읽었습니다.
     DART_API_KEY 확인됨(40자) — 국장 조회가 됩니다.
   ```

> **메모장을 쓰면 함정이 셋입니다.** ① `notepad .env` 는 **메모장을 닫을 때까지 터미널을
> 붙잡아서**, 그동안 친 명령이 전부 메모장 안으로 들어갑니다. ② 파일 형식이 "텍스트 문서"
> 가 기본이라 `.env.txt` 가 되는데 탐색기는 확장자를 숨겨 눈으로는 `.env` 로 보입니다
> (`./run.sh` 가 발견하면 `mv .env.txt .env` 를 찍어 줍니다). ③ 줄끝에 보이지 않는 문자가
> 붙습니다(읽을 때 떼어 냅니다). `./set-dart-key.sh` 는 셋 다 안 겪습니다.

`.env` 는 `.gitignore` 에 있어 **커밋되지 않습니다.** 한 번 적어 두면 창을 닫았다 열어도
`./run.sh` 가 매번 읽습니다. 키 없이 국장을 조회하면 화면이 그렇게 말해 줍니다 — 조용히
빈 화면이 되지는 않습니다.
