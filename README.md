...
**잡음선.** 무작위로 고른 두 종목의 잔차 상관 분포에서 **상위 1% 지점을 창마다
실측**해 같이 저장하고, 그 아래 값은 화면에서 흐리게 칠합니다. 이론값(Bonferroni)
대신 실측을 쓰는 이유는 잔차에도 섹터 요인이 남아 쌍끼리 독립이 아니기 때문입니다.
`잡음선 위만` 체크를 켜면 세 기간 모두 선을 넘은 종목만 남습니다. **상위권이 통째로
흐리면 "이 종목은 지금 같이 가는 게 없다"는 뜻**이고, 그것도 답입니다.
