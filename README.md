...
**직접 계산하지 않고 받아옵니다.** 시장이 이미 계산해 둔 지표가 있는데 우리가 다시 세면
값이 달라져 검증만 어려워집니다. 원천은 셋이고 각각 대체 경로가 있습니다.

| 원천 | 받아오는 것 | 비고 |
| --- | --- | --- |
| **TradingView 스캐너** | `INDEX:S5FI` 등 브레스 지수 | 화면 아래 위젯과 **같은 값**. 당일치만 주므로 하루 한 줄씩 쌓임 |
| **Yahoo (yfinance)** | RSP/SPY · VIX 기간구조 · HYG/LQD · XLP/SPY · 200일선 이격 | 가격 파생이라 **과거치를 한 번에** 채움 |
| **FRED** | 하이일드 스프레드 `BAMLH0A0HYM2` | 키 불필요 |
| **폴백: 직접 계산** | S&P 500 구성종목 종가로 %>20/50/200일선 | TradingView가 막혔을 때만. 느리지만 **과거 시계열까지 복구** |

각 카드에 **출처 배지**(`tradingview` / `yahoo` / `fred` / `computed`)가 붙어 어느 경로로
들어온 값인지 항상 구분됩니다.

**지표 구성** — 요청하신 8개(핵심)에 확인용 6개, 배경용 6개를 더했습니다.

- **핵심 브레스**: `S5TW`(S&P 20일선 위 %) · **`S5FI`(50일선 위 %, 기준 지표)** ·
  `S5TH`(200일선 위 %) · `NDFI` · `NDTH` · `NCTH` · `ADDN`(NYSE 상승−하락) · `ADRN`(등락비율)
- **확인 지표**: **신고가−신저가**(`MAHN−MALN`) · **맥클렐란 오실레이터/총계지수**(`NYMO`/`NYSI`) ·
  **상승/하락 거래량 비율**(`UVOL÷DVOL` — 9:1 매집일/투매일 판정) ·
  **동일가중/시총가중 20일 추이**(`RSP÷SPY` — 브레스를 값 하나로 압축한 것)
- **리스크 레짐**: VIX 기간구조(`^VIX3M÷^VIX`, 1 미만이면 백워데이션) · 하이일드 스프레드 ·
  방어주 상대강도(`XLP÷SPY`) · S&P·나스닥의 200일선 이격 · VIX · HYG/LQD

왜 이 여섯을 더했는지: `%>이평선`만 보면 **"폭은 넓은데 신저가가 쏟아지는 장"** 과
**"거래량 없이 오르는 장"** 을 구분할 수 없고, 주식 브레스만 보면 **신용 경색을 동반한 하락**
인지 **단순 조정**인지 구분할 수 없습니다.

**0~100 종합점수 + 레짐.** 지표를 각각 0~100으로 정규화해 가중 평균합니다
(`S5FI` 25% / `S5TH` 20% / `S5TW`·신고가−신저가·`RSP/SPY` 각 10% / 나머지 소폭).
값이 없는 지표의 가중치는 나머지에 재분배되고, **커버리지 %** 를 같이 표시해 점수를
얼마나 믿을지 알 수 있게 했습니다. 레짐은 `위험(0~30) · 주의(~45) · 중립(~60) · 양호(~80) ·
과열(80~)` 입니다. **표준 지표가 아니라 이 페이지의 합성치**이므로 참고용입니다.

**다이버전스 자동 판정.** 숫자 나열이 아니라 문장으로 경고를 냅니다.
...
- 주식 브레스는 약한데 하이일드 스프레드는 조용함 → **신용 경색 없는 조정**

각 경고에 근거 수치가 함께 나오므로 눈으로 검증할 수 있습니다.

**스크리너와 함께 쓰기.** 브레스가 **양호**면 베이스·평평·턴어라운드 스크리너의 돌파 세팅을
적극적으로, **주의·위험**이면 같은 세팅이라도 비중을 줄이고 손절을 좁히는 식으로 씁니다.

**갱신 주기.** 일봉 기준이라 하루 한 번이면 충분합니다.
`.github/workflows/breadth.yml` 이 **평일 21:10 UTC(뉴욕 마감 후)** 에 수집해
`data/breadth_us.json`(시계열)과 `data/breadth.json`(뷰)을 커밋합니다. 시크릿은 필요 없습니다.

```bash
python tools/breadth_us.py              # 평소 수집(TradingView + Yahoo + FRED)
python tools/breadth_us.py --compute    # 구성종목 직접 계산까지 강제(과거치 백필, 수 분)
...
> ⚠️ `scanner.tradingview.com` · `query1.finance.yahoo.com` · `fred.stlouisfed.org` 는
> egress allowlist 샌드박스에서 **차단**됩니다. 실제 수집은 GitHub Actions에서 돌아가고,
> 판정 로직은 `tests/test_breadth.py` 로 오프라인 단위테스트합니다.
>
> 처음 워크플로를 돌리기 전에는 데이터가 없어 카드가 모두 `—` 로 보입니다(정상).
> **Actions → "Fetch US market breadth" → Run workflow** 에서 `compute=true` 로 한 번
> 돌리면 과거 시계열까지 채워집니다.

**국장 버전은 다음 단계입니다.** `INDEX:S5FI` 같은 기성 브레스 지수가 국장에는 없어서
KOSPI·KOSDAQ 구성종목으로 **직접 계산**해야 합니다. 다행히 이 저장소는 이미 국장 전 종목
OHLCV를 훑는 스크리너(`app/krbase`, `app/krhighs`)가 있어 같은 시계열 포맷
(`data/breadth_kr.json`)으로 채워 넣으면 이 화면을 거의 그대로 재사용할 수 있습니다.
