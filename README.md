...
당연합니다 — 테마 동료가 아니라 자기 자신입니다(NVDA 의 잔차 상위 3개가 FBCG ·
IUSG · SMH 였습니다). 체크를 풀면 다시 보입니다. ETF 는 Finviz 에서 섹터가 전부
`Financial` 로 붙어 나오므로 섹터 자리에 `ETF` 를 써서 섹터 필터가 망가지지 않게 합니다.
