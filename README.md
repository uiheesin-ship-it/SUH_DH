...
**ETF 는 기본으로 숨기고, 저장 자리도 따로 떼어 둡니다.** 성장주 ETF 는 그 종목
자체를 담고 있어 상관이 높은 게 당연합니다 — 테마 동료가 아니라 자기 자신입니다.
화면에서 숨기는 것만으로는 부족합니다. **저장 단계에서 자리를 안 나누면 ETF 가 정원을
통째로 먹기 때문입니다** — 유니버스 상한을 풀어 ETF 가 105개에서 743개로 늘자
MU 의 이웃 60개 중 **45개가 ETF** 가 됐습니다(실제 종목 15개). 지금은 ETF 를
`ETF_SLOTS`(10)개까지만 받고 나머지는 실제 종목에 줍니다.

자리를 마지막에 자를 때만 거는 걸로는 안 됩니다. **후보를 모으는 단계에서 이미 상위
30개가 전부 ETF** 라, 진짜 동료는 후보에도 못 들어 자를 것 자체가 없습니다. 그래서
후보도 ETF 와 실제 종목을 나눠 뽑습니다. 합성 검증에서 진짜 동료 6개가
**0/6 → 6/6** 으로 돌아왔습니다.

ETF 를 아예 빼지 않는 이유는 몇 개는 정보이기 때문입니다 — SMH·SOXX 가 같이 뜨면
"섹터 전체가 움직였다"는 뜻입니다. ETF 는 Finviz 에서 섹터가 전부 `Financial` 로
붙어 나오므로 섹터 자리에 `ETF` 를 써서 섹터 필터가 망가지지 않게 합니다.

**헤지 자리는 따로 뺍니다.** 종목당 60개를 저장하는데, 후보를 잔차 내림차순으로
한 번에 자르면 애써 모은 음의 상관 후보가 전부 잘립니다 — 첫 스냅샷에서 이웃이
꽉 찬 종목의 **68%가 음의 상관 이웃을 하나도 갖지 못했습니다.** 지금은 동행 45자리 ·
헤지 15자리로 나눠 각각 채웁니다.
...
**왜 종목당 60개만 저장하나 — 계산이 아니라 파일 크기 때문입니다.** 상관 자체는
전부 계산합니다(3,000종목 상관행렬이 1초 미만). 문제는 **저장**입니다:

```
  지금  종목당 60개   쌍 181,619개   5.0MB
  전부  종목당 3,026개  쌍 9,159,702개  약 251MB (대칭 절반만 해도 126MB)
```

쌍 하나가 JSON 으로 27.5바이트인데, 전부 저장하면 브라우저가 한 번에 내려받아
파싱해야 할 파일이 250MB 가 됩니다. 60은 거기서 나온 숫자입니다. 늘리고 싶으면
`MAX_NEIGHBORS` 를 올리면 되고, 대략 **종목당 +20개마다 +1.7MB** 입니다.

**비용.** 무거운 건 계산이 아니라 가격 수신입니다 — 2,400종목 × 250일 상관행렬이
**1초 미만**, 이웃 추리기까지 6초입니다. 결과는 종목마다 이웃 약 60개만 남겨
압축 포맷(티커 인덱스 + 상관 ×100 정수)으로 저장하므로 파일이 3~4MB,
전송은 gzip 으로 1.5MB 수준입니다. 화면은 이 파일 하나를 받아 들고 있다가
티커를 바꿀 때 클라이언트에서 펼치므로 **종목을 바꿔도 네트워크 왕복이 없습니다.**
